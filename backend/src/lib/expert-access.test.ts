...
import { activeExpertSubscriptionWhere, EXPERT_SUB_GRACE_MS, isExpertSubscriptionActive } from "./expert-access";
...
describe("isExpertSubscriptionActive", () => {
  ...
  it("refuse un abonnement arrêté (EXPIRED), même avec une échéance future", () => {
    ...
    expect(isExpertSubscriptionActive({ subStatus: "EXPIRED", subExpiresAt: future }, NOW)).toBe(false);
  });

  it("accepte un abonnement ACTIVE dont l'échéance n'est pas passée", () => {
    const future = new Date(NOW.getTime() + DAY);
    expect(isExpertSubscriptionActive({ subStatus: "ACTIVE", subExpiresAt: future }, NOW)).toBe(true);
  });

  it("laisse un délai de grâce après l'échéance, le temps du renouvellement Stripe", () => {
    const justPast = new Date(NOW.getTime() - 2 * DAY);
    expect(isExpertSubscriptionActive({ subStatus: "ACTIVE", subExpiresAt: justPast }, NOW)).toBe(true);
  });
  ...
  it("refuse un abonnement ACTIVE dont l'échéance dépasse le délai de grâce", () => {
    ...
    expect(isExpertSubscriptionActive({ subStatus: "ACTIVE", subExpiresAt: longPast }, NOW)).toBe(false);
  });
